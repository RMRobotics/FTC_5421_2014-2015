...
typedef enum GrabMedTubeStates {
	...
	STATE_DRIVECRASHTUBE,
	STATE_GRABTUBE,
	STATE_DRIVETOWARDPARKINGZONE,
	STATE_LIFT,
	STATE_DROPBUCKET,
	STATE_DROP,
	STATE_END,
		...
		if (rest) {
			...
			switch(currentState){
				...
				case STATE_DRIVETOWARDTUBE:
					writeDebugStream("State: To tube\n");
					driveSetEncoderS(&desiredEncVals, 7.236799 * ENC_PER_REV);
					driveSetMecMotorS(&desiredMotorVals, 0.5);
					...
					break;
				case STATE_CRASHTUBE:
					writeDebugStream("State: CRASH tube\n");
					driveSetEncoderS(&desiredEncVals, .459239 * ENC_PER_REV);
					driveSetMecMotorS(&desiredMotorVals, 0.5);
					...
					if(driveMecHasHitEncoderTarget(&desiredEncVals)){
						restMecMotors();
						currentState = STATE_GRABTUBE;//STATE_DRIVETOWARDCENTERPIECE;
					}
					break;
			.559239
			case STATE_GRABTUBBE:
				writeDebugStream("State: GRAB tube\n");
				servoSetNonCont(TubeGrabber,servoDefinition[TubeGrabber].maxValue);
				if(driveMecHasHitEncoderTarget(&desiredEncVals)){
					restMecMotors();
					currentState = STATE_END;//STATE_DRIVETOWARDCENTERPIECE;
				}
				break;
			case STATE_DRIVETOWARDPARKINGZONE:
				writeDebugStream("State: To tube\n");
				driveSetEncoderN(&desiredEncVals, 7.696038 * ENC_PER_REV);
